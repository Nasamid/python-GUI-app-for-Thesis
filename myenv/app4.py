...
class MainScreen(Screen):
    ...
    def on_zoom_change(self, instance, value):
        try:
            sensor_w, sensor_h = self.manager.app.camera.preview_configuration.main["size"]
            new_w = int(sensor_w / value)
            new_h = int(sensor_h / value)
            x = (sensor_w - new_w) // 2
            y = (sensor_h - new_h) // 2
    ...
    def capture_image(self, instance):
        ...
        raw_path = os.path.join(app.gallery_folder, filename)
        app.image_counter += 1

        # switch to still (high-res) mode, capture, then back to preview
        app.camera.switch_mode_and_capture_file(app.still_conf, raw_path)
        ...
        def process_image():
            try:
                ...
                with open(raw_path, 'rb') as i:
                    inp = i.read()
                out = remove(inp)
                proc_fname = f"processed_{filename[:-4]}.png"
                proc_path = os.path.join(app.gallery_folder, proc_fname)
                with open(proc_path, 'wb') as o:
                    o.write(out)

                # Classification
                diag, conf = app.classify_image(proc_path)
...
class ResultSlide(BoxLayout):
    ...
    def flip_view(self, instance):
        ...
        if self.diagnosis == "Bacterial Blight":
            eng_msg = (
                "Bacterial leaf blight (Xanthomonas oryzae pv. oryzae) detected.\n"
                "- Use certified disease‑free seeds.\n"
                "- Handle seedlings carefully during transplanting to avoid mechanical injury.\n"
                "- Remove infected plant residues to reduce inoculum load.\n"
                "- Apply bactericides such as copper oxychloride or streptomycin sulfate at symptom onset."
            )
            fil_msg = (
                "Nadiskubre ang Bacterial leaf blight (Xanthomonas oryzae pv. oryzae).\n"
                "- Gumamit ng certified disease‑free seeds.\n"
                "- Maingat na paghawak ng punla sa pagtatanim upang maiwasang masugatan.\n"
                "- Alisin ang mga apektadong labi ng halaman upang mabawasan ang inoculum.\n"
                "- Mag-apply ng bactericides tulad ng copper oxychloride o streptomycin sulfate kapag unang lumitaw ang sintomas."
            )

        elif self.diagnosis in ["Blast", "Leaf Blast"]:
            eng_msg = (
                "Leaf Blast (Magnaporthe oryzae) detected.\n"
                "- Cultivate resistant varieties such as IR64 and Pusa 44.\n"
                "- Apply nitrogen judiciously to avoid excessive vegetative growth that encourages infection.\n"
                "- Maintain optimal water management to reduce leaf wetness.\n"
                "- Administer fungicides such as tricyclazole at the booting stage when early symptoms appear."
            )
            fil_msg = (
                "Nadiskubre ang Leaf Blast (Magnaporthe oryzae).\n"
                "- Magtanim ng resistant varieties tulad ng IR64 at Pusa 44.\n"
                "- I-apply ang nitrogen nang maayos upang maiwasan ang sobrang paglaki na naghihikayat ng impeksyon.\n"
                "- Panatilihin ang tamang pamamahala ng tubig upang mabawasan ang basang dahon.\n"
                "- Mag-apply ng fungicides tulad ng tricyclazole sa booting stage kapag lumitaw ang maagang sintomas."
            )

        elif self.diagnosis == "Brown Spot":
            eng_msg = (
                "Brown Spot (Bipolaris oryzae) detected.\n"
                "- Ensure balanced fertilization, especially potassium, to improve plant resilience.\n"
                "- Employ crop rotation with non‑host species to break the disease cycle.\n"
                "- Uphold field hygiene by removing weeds and debris.\n"
                "- Utilize fungicides such as propiconazole or azoxystrobin at disease onset."
            )
            fil_msg = (
                "Nadiskubre ang Brown Spot (Bipolaris oryzae).\n"
                "- Siguraduhing balanseng patabahan, lalo na ng potasa, para sa mas matibay na halaman.\n"
                "- Gamitin ang crop rotation sa mga hindi host upang masira ang disease cycle.\n"
                "- Panatilihing malinis ang taniman sa pamamagitan ng pag-aalis ng damo at labi ng halaman.\n"
                "- Mag-apply ng fungicides tulad ng propiconazole o azoxystrobin kapag nagsimula ang sakit."
            )
        ...
        # Italicize the Filipino message
        if fil_msg:
